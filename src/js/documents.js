...
(function() {
  ...
  async function saveFileToStorage(file, category, fileName) {
    ...
    try {
      ...
      // 使用英文安全路径，避免中文文件夹名导致 Invalid key 错误
      const safeCategory = encodeURIComponent(category).replace(/%/g, '_');
      const safePath = `projects/${window.App.currentProject.id}/documents/${safeCategory}/${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
  ...
  function renderDocRow(doc) {
    ...
    return `
      <tr data-doc-id="${doc.id}" style="cursor:pointer" class="doc-row">
        <td>
  ...
  function bindRowEvents() {
    ...
    document.querySelectorAll('.doc-row').forEach(row => {
      row.addEventListener('click', (e) => {
        // 如果点击的是操作按钮或其子元素，不触发预览
        if (e.target.closest('.doc-action-preview, .doc-action-download, .doc-action-edit, .doc-action-delete')) return;
        previewDoc(row.dataset.docId);
      });
    });
  ...
  async function previewDoc(docId) {
    ...
    if (isPdfType(doc.type)) {
      previewHtml = fileUrl 
        ? `<iframe src="${fileUrl}" width="100%" height="70vh" style="border:none;border-radius:8px;min-height:500px" type="application/pdf"></iframe>`
        : `<div style="text-align:center;color:var(--ink-faint);padding:40px">无法加载 PDF 预览</div>`;
    } else if (isImageType(doc.type)) {
      previewHtml = fileUrl
        ? `<div style="text-align:center"><img src="${fileUrl}" style="max-width:100%;max-height:70vh;border-radius:8px;cursor:zoom-in" onclick="this.style.maxHeight=this.style.maxHeight==='none'?'70vh':'none';this.style.cursor=this.style.cursor==='zoom-out'?'zoom-in':'zoom-out'" alt="${escapeHtml(doc.name)}"></div>`
        : `<div style="text-align:center;padding:20px;color:var(--ink-faint)">无法加载图片</div>`;
    } else if (['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'].includes((doc.type || '').toLowerCase())) {
      // Office 文档使用 Microsoft Office Online Viewer
      const viewerUrl = fileUrl ? `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(fileUrl)}` : null;
      previewHtml = viewerUrl
        ? `<iframe src="${viewerUrl}" width="100%" height="70vh" style="border:none;border-radius:8px;min-height:500px"></iframe>`
        : `<div class="empty-state" style="padding:40px">
            <div class="empty-state-icon">📎</div>
            <div class="empty-state-text">无法在线预览 ${(doc.type || '').toUpperCase()} 文件</div>
            <button class="btn btn-primary btn-sm" id="previewDownloadBtn">下载文件查看</button>
          </div>`;
    } else {
      previewHtml = `
        <div class="empty-state" style="padding:40px">
          <div class="empty-state-icon">📎</div>
          <div class="empty-state-text">暂不支持在线预览，请下载查看</div>
    ...
    content.innerHTML = `
      ...
      <div class="modal-body" id="previewBody" style="padding:0;max-height:75vh;overflow:auto">
        ${previewHtml}
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary btn-sm" onclick="closeModal()">关闭</button>
        <button class="btn btn-primary btn-sm" id="previewDownloadBtn2">⬇️ 下载</button>
      </div>
    `;
